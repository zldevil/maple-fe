<template>
  <div>
    ...
    >
      <template #queryRight>
        <el-button v-auth="perms.addRole" type="primary" icon="plus" @click="editRole(false)"
          >添加</el-button
        >
        <el-button
          v-auth="perms.delRole"
          :disabled="selectionData.length < 1"
          @click="deleteRole(selectionData)"
          type="danger"
          icon="delete"
          >删除</el-button
        >
      ...
      <template #action="{ data }">
        <el-button v-if="actionBtns[perms.updateRole]" @click="editRole(data)" type="primary" link
          >编辑</el-button
        >
        <el-button
          v-if="actionBtns[perms.saveRoleResource]"
    ...
    <role-edit
      ...
      v-model:visible="roleEditDialog.visible"
      :data="roleEditDialog.role"
...
import { roleApi, resourceApi } from '@/common/sysApi'
...
const { query, total, roles, selectionData, resourceDialog, roleEditDialog, showResourceDialog } =
  toRefs(state)
...
const deleteRole = async (data: any) => {
  try {
    await ElMessageBox.confirm(
      `此操作将删除【${data
        .map((x: any) => x.name)
        .join(', ')}】该角色，以及与该角色有关的账号角色关联信息和资源角色关联信息, 是否继续?`,
      '提示',
      {
