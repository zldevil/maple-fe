<template>
  <div>
    ...
    >
      <template #queryRight>
        <el-button v-auth="perms.addAccount" type="primary" icon="plus" @click="editAccount(false)"
          >添加</el-button
        >
        <el-button
          v-auth="perms.delAccount"
          :disabled="state.selectionData.length < 1"
          @click="deleteAccount()"
          type="danger"
          icon="delete"
          >删除</el-button
      ...
      <template #action="{ data }">
        ...
        <el-button
          link
          v-if="actionBtns[perms.changeAccountStatus] && data.status == 1"
      ...
      >
        <template #default="{ node, data }">
          <span class="custom-tree-node">
            ...
            <span v-if="data.type == ResourceTypeEnum.Permission.value" style="color: #67c23a">{{
              node.label
            }}</span>
          </span>
        </template>
    ...
    <role-edit
      ...
      :account="roleDialog.account"
      @cancel="cancel()"
    />
    <account-edit
      v-model:visible="accountDialog.visible"
      v-model:account="accountDialog.data"
...
import { AccountStatusEnum, ResourceTypeEnum } from '@/common/enums'
import { accountApi } from '@/common/sysApi'
...
const deleteAccount = async () => {
  try {
    await ElMessageBox.confirm(
      `确定删除【${state.selectionData.map((x: any) => x.name).join(', ')}】的账号?`,
      '提示',
      {
