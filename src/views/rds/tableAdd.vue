          ...
          <el-col :span="12">
            <el-form-item prop="characterSet" label="charset">
              <el-select
                filterable
                style="width: 80%"
                v-model="tableData.characterSet"
                size="small"
              >
                <el-option
        ...
        <el-tabs v-model="activeName">
          <el-tab-pane label="字段" name="1">
            <el-table :data="tableData.fields.res" :max-height="tableData.height">
              ...
              >
                <template #default="scope">
                  ...
                  >
                    <el-option
                      v-for="typeValue in columnTypeList"
                      :key="typeValue"
                      :value="typeValue"
                      >{{ typeValue }}</el-option
                    >
            ...
            <el-row style="margin-top: 20px">
              <el-button @click="addDefaultRows()" link type="warning" icon="plus"
                >添加默认列</el-button
              >
          ...
          <el-tab-pane label="索引" name="2">
            <el-table :data="tableData.indexs.res" :max-height="tableData.height">
              <el-table-column
                :prop="item.prop"
                :label="item.label"
                v-for="item in tableData.indexs.colNames"
                :key="item.prop"
              >
                <template #default="scope">
                  <el-input
                    v-if="item.prop === 'indexName'"
                    size="small"
                    disabled
                  ...
                  >
                    <el-option
                      v-for="cl in tableData.indexs.columns"
                      :key="cl.name"
                      :label="cl.name"
                      :value="cl.name"
                    >
                  ...
                  <el-select
                    v-if="item.prop === 'indexType'"
                    filterable
                    size="small"
                    v-model="scope.row.indexType"
                  >
                    <el-option
                      v-for="typeValue in indexTypeList"
                      :key="typeValue"
                      :value="typeValue"
                      >{{ typeValue }}</el-option
                    >
  ...
  if (!props.data?.edit) {
    if (state.activeName === '1') {
      ...
      return `CREATE TABLE ${data.tableName}
                  ...
                  ) ENGINE=InnoDB DEFAULT CHARSET=${data.characterSet} COLLATE =${
                    data.collation
      ...
      state.tableData.indexs.res.forEach((a) => {
        sql += ` ADD ${a.unique ? 'UNIQUE' : ''} INDEX ${a.indexName}(${a.columnNames.join(
          ','
        )}) USING ${a.indexType} COMMENT '${a.indexComment}',`
      })
