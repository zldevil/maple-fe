<template>
  <div class="db-list">
    ...
    >
      ...
      <template #queryRight>
        <el-button v-auth="perms.saveDb" type="primary" icon="plus" @click="editDb(false)"
          >添加</el-button
        >
        <el-button
          v-auth="perms.delDb"
      ...
      <template #database="{ data }">
        <el-popover placement="right" trigger="click" :width="300">
          <template #reference>
            <el-link type="primary" :underline="false" plain @click="selectDb(data.dbs)"
              >查看
            </el-link>
          </template>
          <el-input
            v-model="filterDb.param"
            @keyup="filterSchema"
            class="w-50 m-2"
            placeholder="搜索"
            size="small"
          ...
          <div
            ...
            :key="db"
            style="
              border: 1px var(--color-success-light-3) solid;
              margin-top: 3px;
              border-radius: 5px;
              padding: 2px;
              position: relative;
      ...
      <template #more="{ data }">
        ...
        <el-button class="ml5" type="primary" @click="onShowSqlExec(data)" link
          >SQL执行记录</el-button
        >
      </template>

      <template #action="{ data }">
        <el-button v-if="actionBtns[perms.saveDb]" @click="editDb(data)" type="primary" link
          >编辑</el-button
        >
      </template>
    </page-table>
    ...
    >
      <el-row class="mb10">
        <el-popover v-model:visible="showDumpInfo" :width="470" placement="right" trigger="click">
          ...
          <el-form-item label="导出表: ">
            <el-table
              @selection-change="handleDumpTableSelectionChange"
              max-height="300"
              size="small"
      ...
      <el-table
        v-loading="tableInfoDialog.loading"
        border
        stripe
        :data="filterTableInfos"
        size="small"
        max-height="680"
      >
        <el-table-column property="tableName" label="表名" min-width="150" show-overflow-tooltip>
          <template #header>
            <el-input
              v-model="tableInfoDialog.tableNameSearch"
              size="small"
              placeholder="表名: 输入可过滤"
              clearable
            />
          </template>
        </el-table-column>
        <el-table-column property="tableComment" label="备注" min-width="150" show-overflow-tooltip>
          <template #header>
            <el-input
              v-model="tableInfoDialog.tableCommentSearch"
              size="small"
              placeholder="备注: 输入可过滤"
              clearable
            />
          </template>
        </el-table-column>
        ...
        <el-table-column label="更多信息" min-width="140">
          <template #default="scope">
            ...
            <el-link class="ml5" @click.prevent="showTableIndex(scope.row)" type="success"
              >索引</el-link
            >
            <el-link
              class="ml5"
    ...
    >
      ...
      >
        <template #dbSelect>
          <el-select
            v-model="sqlExecLogDialog.query.db"
            placeholder="请选择数据库"
            style="width: 200px"
            filterable
            clearable
          >
            <el-option
              v-for="item in sqlExecLogDialog.dbs"
              :key="item"
              :label="`${item}`"
              :value="item"
            >
        ...
        <template #action="{ data }">
          <el-link
            v-if="
              data.type == DbSqlExecTypeEnum.Update.value ||
              data.type == DbSqlExecTypeEnum.Delete.value
            "
            type="primary"
            plain
            size="small"
    ...
    <el-dialog width="40%" :title="`${chooseTableName} 字段信息`" v-model="columnDialog.visible">
      <el-table border stripe :data="columnDialog.columns" size="small">
        ...
        <el-table-column width="120" prop="columnType" label="类型" show-overflow-tooltip>
        </el-table-column>
        <el-table-column width="80" prop="nullable" label="是否可为空" show-overflow-tooltip>
        </el-table-column>
    ...
    <el-dialog width="40%" :title="`${chooseTableName} 索引信息`" v-model="indexDialog.visible">
      <el-table border stripe :data="indexDialog.indexs" size="small">
        ...
        <el-table-column prop="columnName" label="列名" min-width="120" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="seqInIndex" label="列序列号" show-overflow-tooltip>
        </el-table-column>
    ...
    <el-dialog v-model="infoDialog.visible">
      <el-descriptions title="详情" :column="3" border>
        ...
        <el-descriptions-item :span="1.5" label="名称">{{
          infoDialog.data.name
        }}</el-descriptions-item>

        <el-descriptions-item :span="3" label="标签路径">{{
          infoDialog.data.tagPath
        }}</el-descriptions-item>

        <el-descriptions-item :span="2" label="主机">{{
          infoDialog.data.host
        }}</el-descriptions-item>
        <el-descriptions-item :span="1" label="端口">{{
          infoDialog.data.port
        }}</el-descriptions-item>

        <el-descriptions-item :span="2" label="用户名">{{
        ...
        <el-descriptions-item :span="3" label="连接参数">{{
          infoDialog.data.params
        }}</el-descriptions-item>
        <el-descriptions-item :span="3" label="备注">{{
          infoDialog.data.remark
        }}</el-descriptions-item>
        <el-descriptions-item :span="3" label="数据库">{{
          infoDialog.data.database
        }}</el-descriptions-item>

        <el-descriptions-item :span="3" label="SSH隧道"
        ...
        <el-descriptions-item :span="1" label="创建者">{{
        ...
        <el-descriptions-item :span="1" label="修改者">{{
...
import { DbSqlExecTypeEnum } from '@/common/enums'
...
const columns = ref([
  ...
  TableColumn.new('host', 'host:port').setFormatFunc(
    (data: any, _prop: string) => `${data.host}:${data.port}`
  ),
...
const actionColumn = TableColumn.new('action', '操作')
  .isSlot()
  .setMinWidth(65)
  .fixedRight()
  .alignCenter()
...
const deleteDb = async () => {
  try {
    await ElMessageBox.confirm(
      `确定删除【${state.selectionData.map((x: any) => x.name).join(', ')}】库?`,
      '提示',
      {
...
const dump = (db: string) => {
  ...
  a.setAttribute(
    'href',
    `${config.baseApiUrl}/dbs/${state.dbId}/dump?db=${db}&type=${
      state.dumpInfo.type
    }&tables=${state.dumpInfo.tables.join(',')}&token=${getSession('token')}`
  )
  a.click()
...
const onShowRollbackSql = async (sqlExecLog: any) => {
  ...
  if (sqlExecLog.type == DbSqlExecTypeEnum['UPDATE'].value) {
    for (let ov of oldValue) {
      ...
      rollbackSqls.push(
        `UPDATE ${sqlExecLog.table} SET ${setItems.join(', ')} WHERE ${primaryKey} = ${wrapValue(
          ov[primaryKey]
        )};`
      )
    }
    ...
    for (let ov of oldValue) {
      ...
      rollbackSqls.push(
        `INSERT INTO ${sqlExecLog.table} (${columnNames.join(', ')}) VALUES (${values.join(', ')});`
      )
    }
  }
  ...
  try {
    ...
    SqlExecBox({
      ...
      runSuccessCallback: async () => {
        state.tableInfoDialog.infos = await dbApi.tableInfos.request({
          id: state.dbId,
          db: state.db
        })
      }
