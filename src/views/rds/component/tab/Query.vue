      ...
      <div class="toolbar">
        <div class="fl">
          ...
          <el-tooltip class="box-item" effect="dark" content="limit" placement="top">
            <el-link @click="onLimit()" type="success" :underline="false" icon="Operation">
            </el-link>
          </el-tooltip>
        </div>

        <div style="float: right" class="fl">
          <el-button @click="saveSql()" type="primary" icon="document-add" plain size="small"
            >保存SQL
          </el-button>
          <el-button
            v-if="sqlName"
    ...
    <div class="mt5">
      <el-row>
        ...
        <span v-if="execRes.data.length > 0">
          <el-divider direction="vertical" border-style="dashed" />
          <el-link type="success" :underline="false" @click="exportData"
            ><span style="font-size: 12px">导出</span></el-link
          >
        </span>
        <span v-if="hasUpdatedFileds">
          <el-divider direction="vertical" border-style="dashed" />
          <el-link type="success" :underline="false" @click="submitUpdateFields()"
            ><span style="font-size: 12px">提交</span></el-link
          >
        </span>
        <span v-if="hasUpdatedFileds">
          <el-divider direction="vertical" border-style="dashed" />
          <el-link type="warning" :underline="false" @click="cancelUpdateFields"
...
import { nextTick, watch, onMounted, reactive, toRefs, ref } from 'vue'
...
const { tableDataHeight, editorHeight, ti, execRes, table, sqlName, loading, hasUpdatedFileds } =
  toRefs(state)
...
onMounted(async () => {
  ...
  if (params && params.sqlName) {
    ...
    const res = await dbApi.getSql.request({
      id: state.ti.dbId,
      type: 1,
      name: state.sqlName,
      db: state.ti.db
...
const onDragSetHeight = () => {
  document.onmousemove = (e) => {
    ...
    //得到鼠标拖动的宽高距离：取绝对值
    state.editorHeight = `${
      document.getElementById('MonacoTextarea-' + state.ti.key)!.offsetHeight + e.movementY
    }px`
...
const onDeleteData = async () => {
  ...
  dbInst.promptExeSql(
    db,
    dbInst.genDeleteByPrimaryKeysSql(db, state.table, deleteDatas),
    null,
    () => {
      state.execRes.data = state.execRes.data.filter(
        (d: any) =>
          !(
            deleteDatas.findIndex((x: any) => x[primaryKeyColumnName] == d[primaryKeyColumnName]) !=
            -1
          )
      )
      state.selectionDatas = []
