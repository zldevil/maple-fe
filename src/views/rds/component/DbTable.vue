<template>
  <div>
    <el-table
      @cell-dblclick="
        (row: any, column: any, cell: any, event: any) => cellClick(row, column, cell)
      "
...
const { tableHeight, datas, sortable, loading, showColumnTip } = toRefs(state)
...
const cellClick = (row: any, column: any, cell: any) => {
  ...
  if (div && div.tagName === 'DIV') {
    ...
    input.setAttribute(
      'style',
      'height:23px;text-align:center;border:none;' + div.getAttribute('style')
    )
    ...
    input.addEventListener('blur', async () => {
      ...
      if (input.value !== text) {
        ...
        // 被修改的字段
        const primaryKeyFields = currentUpdatedFields.filter(
          (meta) => meta.primaryKey === primaryKeyValue
        )
        let hasKey = false
            ...
            if (a.primaryKey === primaryKeyValue) {
              a.fields =
                a.fields && a.fields.length > 0
                  ? a.fields.filter((f) => f.fieldName !== column.property)
                  : []
...
const submitUpdateFields = () => {
  ...
  DbInst.getInst(state.dbId).promptExeSql(
    db,
    res,
    () => {},
    () => {
