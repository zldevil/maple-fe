<template>
  <div>
          ...
          <template #prefix="{ data }">
            <span v-if="data.type == NodeType.DbInst">
              ...
              >
                <template #reference>
                  <SvgIcon
                    v-if="data.params.type === 'mysql'"
                    name="iconfont icon-op-mysql"
                    :size="18"
                  />
                  <SvgIcon
                    v-if="data.params.type === 'postgres'"
                    name="iconfont icon-op-postgres"
                    :size="18"
                  />
            ...
            <SvgIcon
              name="Files"
              v-if="data.type == NodeType.SqlMenu || data.type == NodeType.Sql"
              color="#f56c6c"
            />
          </template>
        </tag-tree>
    ...
    <el-dialog
      @close="state.genSqlDialog.visible = false"
      v-model="state.genSqlDialog.visible"
      title="SQL"
      width="1000px"
    >
...
const sqlCompletionBuiltinFunctions = [
  ...sqlLanguage.builtinFunctions,
  ...addSqlLanguage.builtinFunctions
]
const sqlCompletionBuiltinVariables = [
...
const loadNode = async (node: any) => {
  ...
  if (nodeType === NodeType.Db) {
    return [
      new TagTreeNode(`${params.id}.${params.db}.table-menu`, '表', NodeType.TableMenu).withParams(
        params
      ),
      new TagTreeNode(getSqlMenuNodeKey(params.id, params.db), 'SQL', NodeType.SqlMenu).withParams(
        params
      )
    ]
  }
...
const nodeClick = async (data: any) => {
  ...
  // 点击数据库，修改当前数据库信息
  if (
    dataType === NodeType.Db ||
    dataType === NodeType.SqlMenu ||
    dataType === NodeType.TableMenu ||
    dataType === NodeType.DbInst
  ) {
    changeSchema(
      {
        id: params.id,
  ...
  if (dataType === NodeType.Sql) {
    await addQueryTab(
      { id: params.id, nodeKey: nodeKey, dbs: params.dbs },
      params.db,
      params.sqlName
    )
  }
...
const getTables = async (params: any) => {
  const { id, db } = params
  let tables = await RdsIns.getInst(id).loadTables(db, state.reloadStatus)
  state.reloadStatus = false
  return tables.map((x: any) => {
    return new TagTreeNode(`${id}.${db}.${x.tableName}`, x.tableName, NodeType.Table)
      .withIsLeaf(true)
      .withParams({
        id,
        db,
...
const loadSqls = async (id: any, db: string, dbs: any) => {
  ...
  return sqls.map((x: any) => {
    return new TagTreeNode(`${id}.${db}.${x.name}`, x.name, NodeType.Sql)
      .withIsLeaf(true)
      .withParams({
        id,
        db,
...
const changeSchema = (inst: any, schema: string) => {
  state.nowDbInst = RdsIns.getOrNewInst(inst)
  state.db = schema
}
...
const onTabChange = () => {
  if (!state.activeName) {
    state.nowDbInst = {} as RdsIns
    state.db = ''
    return
  }
  const nowTab = state.tabs.get(state.activeName)
  state.nowDbInst = RdsIns.getInst(nowTab?.dbId)
  ...
  self.completionItemProvider =
    ...
    monaco.languages.registerCompletionItemProvider('sql', {
      ...
      provideCompletionItems: async (
        ...
        const dbInst = RdsIns.getInst(dbId)
