<template>
  ...
  <div>
    <el-row :gutter="4">
      <el-col :span="4">
        <el-row type="flex" justify="space-between">
          <el-col :span="24" class="el-scrollbar flex-auto">
            <TagTree @node-click="nodeClick" :load="loadNode">
        ...
        <div class="key-list-vtree">
          ...
          <div ref="rightMenuRef" class="key-list-right-menu">
            ...
            <div v-else>
              <el-row>
                <el-link
                  @click="showKeyDetail(state.rightClickNode.key, true)"
                  type="primary"
                  icon="plus"
                  :underline="false"
                  >新tab打开</el-link
              ...
              <el-row class="mt5">
                <el-link
                  @click="delKey(state.rightClickNode.key)"
                  v-auth="'redis:data:del'"
                  type="danger"
                  icon="delete"
                  :underline="false"
      ...
      <el-col :span="11" style="border-left: 1px solid var(--el-card-border-color)">
        <div class="ml5">
          <el-tabs @tab-remove="removeDataTab" style="width: 100%" v-model="state.activeName">
            <el-tab-pane
              closable
              v-for="dt in state.dataTabs"
              :key="dt.key"
              :label="dt.label"
              :name="dt.key"
            >
    ...
    >
      <el-form ref="keyForm" label-width="auto">
        ...
        <el-form-item prop="type" label="类型">
          <el-select
            v-model="newKeyDialog.keyInfo.type"
...
import { redisApi } from '@/common/redisApi'
...
const getDbs = async (redisInfo: any) => {
  ...
  const res = await redisApi.redisInfo.request({
    id: redisInfo.id,
    host: redisInfo.host,
    section: 'Keyspace'
  })
...
const setKeyList = (keys: any) => {
  state.keyTreeData = state.keySeparator
    ? keysToTree(keys, state.keySeparator, state.keyTreeExpanded)
    : keysToList(keys)
  // state.keyTreeData =  keysToList(keys);
  console.log(state.keyTreeData)
  nextTick(() => {
    // key长度小于指定数量，则展开所有节点
    if (keys.length <= 20) {
...
const showKeyDetail = async (key: any, newTab = false) => {
  ...
  if (typeof key == 'object') {
    ...
    const res = await redisApi.keyInfo.request({
