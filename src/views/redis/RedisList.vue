  ...
  <div>
    ...
    >
      ...
      <template #more="{ data }">
        ...
        <el-button
          v-if="data.mode === 'standalone' || data.mode === 'sentinel'"
          type="primary"
          @click="showInfoDialog(data)"
          link
          >单机信息</el-button
        >
        <el-button
          @click="onShowClusterInfo(data)"
          v-if="data.mode === 'cluster'"
          type="primary"
          link
          >集群信息</el-button
        >
    ...
    <el-dialog width="1000px" title="集群信息" v-model="clusterInfoDialog.visible">
      <el-input
        ...
        v-model="clusterInfoDialog.info"
      >
      </el-input>
    ...
    <el-dialog v-model="detailDialog.visible">
      <el-descriptions title="详情" :column="3" border>
        <el-descriptions-item :span="1.5" label="id">{{
        ...
        <el-descriptions-item :span="3" label="标签路径">{{
          detailDialog.data.tagPath
        }}</el-descriptions-item>

        <el-descriptions-item :span="3" label="主机">{{
          detailDialog.data.host
        }}</el-descriptions-item>
...
const deleteRedis = async () => {
  try {
    await ElMessageBox.confirm(
      `确定删除该【${state.selectionData.map((x: any) => x.name).join(', ')}】redis信息?`,
      '提示',
      {
