<template>
  <div>
    ...
    >
      <template #queryRight>
        <el-button v-auth="perms.saveConfig" type="primary" icon="plus" @click="editConfig(false)"
          >添加</el-button
        >
      </template>
      ...
      <template #action="{ data }">
        <el-button
          ...
          type="warning"
          @click="showSetConfigDialog(data)"
          link
          >配置</el-button
        >
        <el-button v-if="actionBtns[perms.saveConfig]" @click="editConfig(data)" type="primary" link
          >编辑
        </el-button>
      </template>
    </page-table>
    ...
    <el-dialog
      :before-close="closeSetConfigDialog"
      title="配置项设置"
    ...
    >
      <el-form
        ...
        ref="paramsFormRef"
        :model="paramsDialog.params"
        label-width="auto"
      >
        <el-form-item
          v-for="item in paramsDialog.paramsFormItem"
          :key="item.name"
          :prop="item.model"
          :label="item.name"
          required
      ...
      <el-form v-else ref="paramsFormRef" label-width="auto">
        <el-form-item label="配置值" required>
          <el-input
            ...
            :placeholder="paramsDialog.config.remark"
            autocomplete="off"
            clearable
          ></el-input>
        </el-form-item>
    ...
    <config-edit
      ...
      v-model:visible="configEdit.visible"
      :data="configEdit.config"
...
import { configApi } from '@/common/sysApi'
