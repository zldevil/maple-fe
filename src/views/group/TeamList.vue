  ...
  <div>
    ...
    >
      <template #queryRight>
        <el-button
          ...
          type="primary"
          icon="plus"
          @click="showSaveTeamDialog(false)"
          >添加</el-button
        >
        <el-button
          v-auth="'team:del'"
      ...
      <template #action="{ data }">
        ...
        <el-button
          v-auth="'team:save'"
          @click.prevent="showSaveTeamDialog(data)"
          link
          type="warning"
          >编辑</el-button
        >
    ...
    <el-dialog width="50%" :title="showMemDialog.title" v-model="showMemDialog.visible">
      ...
      <el-dialog
        width="400px"
        title="添加成员"
        :before-close="cancelAddMember"
        v-model="showMemDialog.addVisible"
      >
        <el-form :model="showMemDialog.memForm" label-width="auto">
          <el-form-item label="账号:">
            ...
            >
              <el-option
                v-for="item in showMemDialog.accounts"
                :key="item.id"
...
import { tagApi } from '@/common/groupApi'
...
const { query, addTeamDialog, total, data, selectionData, showMemDialog, showTagDialog } =
  toRefs(state)
...
const deleteTeam = () => {
  ElMessageBox.confirm(
    `此操作将删除【${state.selectionData.map((x: any) => x.name).join(', ')}】团队信息, 是否继续?`,
    '提示',
    {
